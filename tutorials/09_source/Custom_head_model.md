...
implemented in [Fieldtrip](https://www.fieldtriptoolbox.org/tutorial/electrode/). Our goal is to provide
...
If you have scanned electrode positions and wish to use them for source localization, you may follow the [DIPFIT settings](Model_Settings.md) tutorial.

Custom head model
=================
If you have the scanned electrode positions and also happen to have the subjects' MRI, then you may build a custom head model. Sometimes, researchers will remove the face of the subject from the MRI for privacy purposes. It is preferable to use the non-defaced MRI. The first reason is that it will not be possible to precisely select fiducials in the MRI as the nasion is often missing. Even if the position of the nasion is provided, the second reason for not using a defaced MRI is that you will not be able to extract the surface of the face of the subject. These surfaces influence volume conduction. Nevertheless, we will use a defaced MRI in this tutorial with the position of the nasion and other fiducials provided by the experimenters.
...
The script in this section assumes that you have installed the following plugins from the EEGLAB plugin manager: File-IO, Fieldtrip, Picard, and bids-matlab-tools. We first assume you have defined the following file names for the raw data and anatomical MRI.
...
The first step is to import the data. In the script below, we assume that you select EEG channels. However, you may select MEG channels as well as shown in the tutorial script. Source localization will work with both EEG and MEG channels, although not simultaneously. The coordinate of the fiducials are stored in the *coordsystem* JSON file and is automatically detected and imported. However, if you use another file format, they may be included in your data file. 
