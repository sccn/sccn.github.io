...
if((!empty($arguments['fm']) && $arguments['fm'] == 'false') OR (empty($arguments['fm']) && $format == 'markdown_github')) {
    $add_meta = true;
} else {
    $add_meta = false;
}
...
while(list( , $node) = each($result)) {
    ...
    $title = str_replace(':','',$title[0]); // remove : in wiki page title
    $url = str_replace(':','',str_replace(' ', '_', $title));
...
//        $frontmatter .= "permalink: /$url.html\n";
        $frontmatter .= "parent: Other documents\n";
        $frontmatter .= "---\n\n";
    }
    ...
    $options = array(
        "from"  => "mediawiki",
        "to"    => $format,
        "extract-media" => "/Users/dtyoung/Desktop/test/mediawiki-to-markdown/output/assets"
    );
//    print_r($options);
